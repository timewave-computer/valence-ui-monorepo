...
export const Config = {
  ...
  fromAmino(object: ConfigAmino): Config {
    const message = createBaseConfig();
    message.modules =
      object.modules?.map((e) => ModuleConfig.fromAmino(e)) || [];
    message.golangBindings =
      object.golang_bindings?.map((e) => GolangBinding.fromAmino(e)) || [];
    return message;
  },
  toAmino(message: Config): ConfigAmino {
    ...
    if (message.modules) {
      obj.modules = message.modules.map((e) =>
        e ? ModuleConfig.toAmino(e) : undefined,
      );
    } else {
      obj.modules = message.modules;
    }
    if (message.golangBindings) {
      obj.golang_bindings = message.golangBindings.map((e) =>
        e ? GolangBinding.toAmino(e) : undefined,
      );
    } else {
...
export const GolangBinding = {
  ...
  toAmino(message: GolangBinding): GolangBindingAmino {
    const obj: any = {};
    obj.interface_type =
      message.interfaceType === "" ? undefined : message.interfaceType;
    obj.implementation =
      message.implementation === "" ? undefined : message.implementation;
    return obj;
  },
